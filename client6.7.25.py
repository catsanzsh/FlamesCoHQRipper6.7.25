"""
CATSEEK R1.0X.X.X [CATSEEK] 20XX - OpenRouter Edition
"""
...
# Constants
# Switched to OpenRouter API with Deepseek model
DEFAULT_MODEL = "deepseek/deepseek-coder-r1-0528-qwen3-8b"
LLM_TIMEOUT = 120
CODE_TIMEOUT = 60
HTTP_REFERER = "http://localhost:8000" # Recommended by OpenRouter
...
def get_api_key() -> str:
    ...
    # The key is now requested exclusively through the UI popup.
    return ""
...
class AutonomousAgent:
    MASTER_TOOL_LIBRARY = {
        ...
        "task_complete": {
            "description": "Shuts down the agent when the goal is fully achieved.",
    ...
    def __init__(self, goal: str, api_client: APIClient, code_interpreter: CodeInterpreter,
        ...
        self.history: List[Dict[str, Any]] = [] # Now uses OpenAI format
    ...
    async def run(self):
        ...
        # Initial messages in OpenAI format
        self.history = [
        ...
        iteration_count = 0
        while not self.stop_event.is_set() and not self.completed:
            iteration_count += 1
            ...
            try:
                ...
                # Build OpenAI/OpenRouter payload
                payload = {
                ...
                self.history.append(message) # Add assistant's response to history
...
class AgentUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("CATSEEK R1.0X.X.X [CATSEEK] 20XX")
        ...
        logger.info("CATSEEK UI initialized.")

    def _setup_async_loop(self):
        """Creates and starts a dedicated thread for the asyncio event loop."""
        if not ASYNC_MODE:
            return
    ...
    def setup_ui(self):
        ...
        self.output_text.tag_configure("system", foreground="#2ecc71") # Green
        self.output_text.tag_configure("agent", foreground="#3498db")  # Blue
        self.output_text.tag_configure("tool", foreground="#f39c12")   # Orange
        self.output_text.tag_configure("result", foreground="#9b59b6") # Purple
        self.output_text.tag_configure("llm", foreground="#95a5a6")    # Gray
        self.output_text.tag_configure("error", foreground="#e74c3c")  # Red
    ...
    def start_agent(self):
        ...
        system_prompt = "You are CATSEEK R1, an autonomous AI agent. Your goal is to solve problems by thinking, planning, and executing commands. You must use the provided tools to achieve the user's goal. When the goal is fully achieved, you MUST call the `task_complete` tool."
        ...
        # Submit the agent's run coroutine to the dedicated asyncio event loop
    ...
    def update_ui(self):
        ...
        # Check if the agent task is running and update UI accordingly
        if self.agent_task_future and self.agent_task_future.done() and self.stop_button['state'] == tk.NORMAL:
            try:
                # Calling .result() will raise an exception if the task failed
            ...
            self.start_button.config(state=tk.NORMAL)
            self.stop_button.config(state=tk.DISABLED)
            self.agent_task_future = None # Reset for the next run
    ...
    def _on_closing(self):
        global RUNTIME_API_KEY
        logger.info("Initiating CATSEEK shutdown")
        self.shutdown_event.set()
        
        if ASYNC_MODE and self.async_loop and self.async_loop.is_running():
            try:
                # Schedule the close session coroutine
                ...
                if self.async_thread:
                    self.async_thread.join(timeout=2) # Wait for thread to finish
